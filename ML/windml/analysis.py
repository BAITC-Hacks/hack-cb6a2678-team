...
def analyze(fc: pd.DataFrame, meta: dict | None = None, model_meta: dict | None = None,
    ...
    def add(level, code, msg):
        (issues if level != "info" else info).append({"level": level, "code": code, "message": msg})

    n = len(fc)
    if n != expected_hours:
        add("error", "WRONG_LENGTH", f"{n} часов вместо {expected_hours}")
    nan = int(fc["forecast"].isna().sum())
    if nan:
        add("error", "NAN_VALUES", f"{nan} часов без прогноза (нет погодных данных)")
    if ((fc["forecast"] < 0) | (fc["forecast"] > 1)).any():
        add("error", "OUT_OF_RANGE", "значения вне [0, 1]")
    if {"p10", "p90"} <= set(fc.columns) and (fc["p10"] > fc["p90"] + 1e-9).any():
        add("error", "QUANTILE_CROSSING", "p10 > p90")
    ...
    if spread is not None:
        hi = int((spread > HIGH_SPREAD_MS).sum())
        if hi:
            add("warning", "HIGH_NWP_DISAGREEMENT",
                f"{hi} ч с разбросом ветра между моделями > {HIGH_SPREAD_MS} м/с — "
                f"рекомендуется пересчёт после следующего обновления прогноза погоды")

    lo_ws, hi_ws = model_meta.get("ws_train_range", [0, 99])
    ws = fc.get("ens_ws_mean")
    if ws is not None and ((ws > hi_ws) | (ws < lo_ws)).any():
        add("warning", "OUT_OF_DISTRIBUTION", f"ветер вне диапазона обучения [{lo_ws:.1f}; {hi_ws:.1f}] м/с")

    ramps = fc["forecast"].diff().abs()
    ramp_hours = [str(t) for t in fc.index[ramps > RAMP_THRESHOLD]]
    if ramp_hours:
        add("info", "RAMP_EVENTS", f"резкие изменения мощности (>{RAMP_THRESHOLD:.0%}/ч): {ramp_hours[:5]}")
    width = (fc["p90"] - fc["p10"]).mean() if "p90" in fc else np.nan
    if width > 0.45:
        add("warning", "WIDE_INTERVAL", f"средняя ширина интервала p10–p90 = {width:.2f}")
    if "pc_baseline" in fc:
        gap = float((fc["forecast"] - fc["pc_baseline"]).abs().mean())
        if gap > 0.2:
            add("warning", "ML_VS_PHYSICS_GAP", f"ML-прогноз сильно отличается от физической кривой мощности ({gap:.2f})")
